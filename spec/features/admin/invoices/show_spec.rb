require 'rails_helper'
  ...
  let!(:invoice_item_1)  {InvoiceItem.create!(item_id: lamp.id, invoice_id: invoice_1.id, quantity: 10, unit_price: 2999, status: "pending")}
  ...
  describe 'admin_invoice#show' do
    ...
    it "has total revenue and total discounted revenue" do
      visit admin_invoice_path(invoice_1)
      within ("#info") do
        expect(page).to have_content("Total Revenue: $#{invoice_1.admin_total_revenue(invoice_1)}")
        expect(page).to have_content("Discounted: #{invoice_1.discounted_revenue(nomi)}")
      end
    end
  end
end
